...
# Local imports
from src.steam_client import get_owned_games, get_game_details, get_player_summary
from src.hltb_client import get_hltb_stats
from src.recommender import get_recommender, AIRecommender
...
async def login_with_steam():
    ...
    params = {
        ...
        "openid.return_to": "http://127.0.0.1:8000/auth/steam/callback",
        "openid.realm": "http://127.0.0.1:8000",
        "openid.identity": "http://specs.openid.net/auth/2.0/identifier_select",
        "openid.claimed_id": "http://specs.openid.net/auth/2.0/identifier_select",
    }
    redirect_url = f"{steam_login_url}?{'&'.join([f'{k}={v}' for k, v in params.items()])}"
...
async def auth_steam_callback(request: Request):
    ...
    if "is_valid:true" in response.text:
        ...
        persona_name = summary.get('personaname', 'Unknown User') if summary else 'Unknown User'
        ...
        return JSONResponse({"status": "error", "message": "Steam authentication failed."})
...
async def api_get_games(request: Request):
    ...
    if cached_steam_data and (time.time() - cached_steam_data['timestamp']) < CACHE_EXPIRATION:
    ...
    if not steam_data:
        return JSONResponse(status_code=500, content={"detail": "Could not fetch games from Steam API"})
...
async def refresh_games_api(request: Request):
    ...
    if steam_data:
        steam_cache[steam_id] = {'timestamp': time.time(), 'data': steam_data}
        return {"status": "success", "message": "Game list refreshed and cache updated."}
    else:
        return JSONResponse(status_code=500, content={"detail": "Could not fetch games from Steam API during refresh."})

# --- AI Recommendation Endpoints ---
async def _get_and_process_games(steam_id: str, exclude_by_hltb: bool, playtime_threshold_hours: int | None) -> List[Dict] | None:
    cached_steam_data = steam_cache.get(steam_id)
    if cached_steam_data and (time.time() - cached_steam_data['timestamp']) < CACHE_EXPIRATION:
        steam_data = cached_steam_data['data']
    else:
    ...
    # --- Filtering Logic ---
    filtered_games = [g for g in games if g.get('appid') not in manually_excluded_appids]
    ...
    if playtime_threshold_hours is not None and playtime_threshold_hours > 0:
        ...
        filtered_games = [g for g in filtered_games if g.get('playtime_forever', 0) <= threshold_minutes]
    ...
    for game in games_to_process:
        ...
        if not (appid and game_name): continue
        ...
        game_details = None
        if cached_details and (time.time() - cached_details['timestamp']) < CACHE_EXPIRATION:
            game_details = cached_details['data']
        else:
            fetched_details = await get_game_details(appid)
            if fetched_details:
                game_details = fetched_details
                app_details_cache[appid] = {'timestamp': time.time(), 'data': game_details}
        ...
        sanitized_games_for_llm.append({
            ...
            "hltb": hltb_stats,
            "genres": [g['description'] for g in game_details.get('genres', [])] if game_details else [],
            "categories": [c['description'] for c in game_details.get('categories', [])] if game_details else [],
        })
    return sanitized_games_for_llm

@app.post("/api/recommendations")
async def get_game_recommendations(request: Request, user_prompt: str = Form(...), custom_model: str = Form(None), custom_prompt: str = Form(None), playtime_threshold_hours: int = Form(None), exclude_by_hltb: bool = Form(False)):
    user = request.session.get('user')
    if not user or not (steam_id := user.get('steam_id')):
        return JSONResponse(status_code=401, content={"detail": "Not authenticated"})

    games_for_llm = await _get_and_process_games(steam_id, exclude_by_hltb, playtime_threshold_hours)
    if games_for_llm is None:
        return JSONResponse(status_code=500, content={"detail": "Could not fetch games from Steam API"})

    recs, metrics = await ai_recommender.get_recommendations(games_for_llm, user_prompt, custom_model, custom_prompt)
    return JSONResponse(content={"recommendations": recs, "metrics": metrics})

@app.post("/api/recommendations/surprise-me")
async def get_surprise_recommendation(request: Request, user_prompt: str = Form("Surprise me!"), custom_model: str = Form(None), custom_prompt: str = Form(None), playtime_threshold_hours: int = Form(None), exclude_by_hltb: bool = Form(False)):
    user = request.session.get('user')
    if not user or not (steam_id := user.get('steam_id')):
        return JSONResponse(status_code=401, content={"detail": "Not authenticated"})
    ...
    rec, metrics = await ai_recommender.surprise_me(games_for_llm, user_prompt, custom_model, custom_prompt)
